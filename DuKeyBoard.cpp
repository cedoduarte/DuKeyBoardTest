...
bool DuKeyBoard::m_created = false;

DuKeyBoard::DuKeyBoard(bool embeddedKeyboard, QWidget *activeForm,
        QWidget *parent)
    : QWidget(parent)
    , m_nextInput(nullptr)
    ...
    , m_clipboard(QGuiApplication::clipboard())
{
    ...
    setZoomFacility(true);
    enableSwitchingEcho(false); // enable possibility to change echo through keyboard
    createKeyboard();
}

DuKeyBoard::~DuKeyBoard()
{
}

void DuKeyBoard::enableSwitchingEcho(bool status)
{
	this->m_enablePasswordEcho = status;
}

bool DuKeyBoard::isEnabledSwitchingEcho(void)
{
	return (this->m_enablePasswordEcho);
}

/*inline */bool DuKeyBoard::isEmbeddedKeyboard(void)
{
	return (this->m_embeddedKeyboard);
}

void DuKeyBoard::setZoomFacility(bool active)
{
	this->m_zoomFacilityEmbedded = active;
}

bool DuKeyBoard::isZoomFacilityEnable(void)
{
	return (this->m_zoomFacilityEmbedded);
}

DuKeyPushButton *DuKeyBoard::createNewKey(QString keyValue)
{
    DuKeyPushButton *tmp = new DuKeyPushButton(this);
	QString style = QString(DEFAULT_STYLE_BUTTON)
	...
	if (IS_PASSWORD(keyValue) == true || IS_PASSWORD_EMB(keyValue) == true) {
        ...
        tmp->setIcon(QPixmap(":/normalEcho"));
	} else if (IS_PASTE(keyValue) == true) {
		tmp->setIconSize(imageDim);
		tmp->setEnabled(false);
        tmp->setIcon(QPixmap(":/paste"));
	} else if (IS_COPY(keyValue) == true) {
		tmp->setIconSize(imageDim);
        tmp->setIcon(QPixmap(":/copy"));
	} else if (IS_CUT_LEFT(keyValue) == true) {
		tmp->setIconSize(imageDim);
        tmp->setIcon(QPixmap(":/cut"));
	}
...
void DuKeyBoard::controlKeyEcho(QLineEdit *control)
{
	QString textToSearch;
    DuKeyPushButton *echoControlKey;
	QString tmpStyle;
    ...
    if (pasteButton != nullptr && copyButton != nullptr) { // support for copy/paste functionality
    ...
    echoControlKey = this->findChild<DuKeyPushButton *>(textToSearch);
    if (echoControlKey != nullptr) { // support for password echo functionality
		if (control->echoMode() == QLineEdit::Normal) {
			echoControlKey->setStyleSheet(
					QString(DEFAULT_STYLE_BUTTON)
							+ QString(DEFAULT_BACKGROUND_BUTTON) + tmpStyle); // segnalato come attivo
            echoControlKey->setIcon(QPixmap(":/normalEcho"));
		} else if (control->echoMode() == QLineEdit::Password) {
			echoControlKey->setStyleSheet(
					QString(DEFAULT_STYLE_BUTTON)
							+ QString(CHANGED_BACKGROUND_BUTTON) + tmpStyle); // segnalato come attivo
            echoControlKey->setIcon(QPixmap(":/passwdEcho"));
		}
...
void DuKeyBoard::receiptChildKey(QKeyEvent *event,
        QLineEdit *focusThisControl, bool reset)
{
    static QLineEdit *nextInput = nullptr;
...
void DuKeyBoard::switchKeyEchoMode(QLineEdit *control)
{
...
QLineEdit *DuKeyBoard::getNextTextbox(QLineEdit *thisControl, bool reset)
{
    QLineEdit *tmpInputFound = nullptr;

    this->m_currentTextBox = nullptr;
    if (this->m_activeWindow == nullptr) { // nessuna finestra principale su cui operare
        m_nextInput = nullptr;
        return (nullptr);
	}
    if (thisControl != nullptr) {
		if (this->m_activeWindow->findChild<QLineEdit *>(
				thisControl->objectName()) == 0) // verifies that thisControl is a child of m_activeWindow
            return (nullptr);
		else
			// it's true!
    ...
    if (m_nextInput == nullptr || reset == true)
	...
	do {
		...
		m_nextInput = m_nextInput->nextInFocusChain();
    } while (m_nextInput != nullptr);
    return (nullptr);
}

QLineEdit *DuKeyBoard::setCurrentTextStyle(QLineEdit *control)
{
...
QLineEdit *DuKeyBoard::setDefaultTextStyle(QLineEdit *control)
{
	control->setStyleSheet(this->m_noFocusPalette.styleSheet());
	return (control);
}

void DuKeyBoard::init_keyboard(QLineEdit *focusThisControl)
{
    this->m_currentTextBox = nullptr;
	this->m_nextInput = focusThisControl;
    this->receiptChildKey(nullptr, focusThisControl, true);
}

void DuKeyBoard::show(QWidget *activeForm)
{
	this->m_activeWindow = activeForm;
    this->init_keyboard(nullptr);
	if (this->windowState() == Qt::WindowMinimized)
		this->setWindowState(Qt::WindowNoState);
	this->setStatusEchoMode(false);
	this->m_clipboard->clear();
	this->move(20,
			QGuiApplication::screens()[0]->availableGeometry().height()
					- this->height() - 200);
	this->borderFrame(true);
	QWidget::show();
}

void DuKeyBoard::borderFrame(bool visible)
{
	if (visible == true)
		this->setWindowFlags(
...
void DuKeyBoard::focusThis(QLineEdit *control)
{
    if (this->m_activeWindow != nullptr) {
        if (this->m_currentTextBox != nullptr)
			this->m_currentTextBox = this->setDefaultTextStyle(
					this->m_currentTextBox);
...
void DuKeyBoard::closeEvent(QCloseEvent *event)
{
    (void) event;
    hide(true);
}

void DuKeyBoard::hide(bool changeColor)
{
	try {
		if (changeColor == true)
            if (this->m_activeWindow != nullptr
...
void DuKeyBoard::createKeyboard(void)
{
    DuKeyPushButton *tmp = nullptr;
	QVBoxLayout *tmpVLayout = new QVBoxLayout;
	QHBoxLayout *tmpLayout = new QHBoxLayout;
    QString tmpStyle = QString::null;

    if (DuKeyBoard::m_created == true) // tastiera già  creata: esce
		return;
    DuKeyBoard::m_created = true; // segnala che è stata creata la tastiera
	//
	...
	if (this->isEmbeddedKeyboard() == true)
		tmpStyle = QString(EMBEDDED_KEYBOARD);
	tmp->setStyleSheet(
			QString(DEFAULT_STYLE_BUTTON) + QString(CHANGED_BACKGROUND_BUTTON)
					+ tmpStyle); // segnalato come attivo
	tmpLayout->addWidget(tmp);
