...
DuKeyPushButton::DuKeyPushButton(QWidget *parent)
    : QPushButton(parent), m_parent(parent)
{
	this->setStyleSheet(
			QString(DEFAULT_STYLE_BUTTON) + QString(DEFAULT_BACKGROUND_BUTTON));
	connect(this, SIGNAL(pressedKey(bool)), SLOT(getKeyPress(bool)));
    if ((static_cast<DuKeyBoard *>(parent))->isEmbeddedKeyboard() == true)
		this->style_embedded = QString(EMBEDDED_KEYBOARD);
}

void DuKeyPushButton::getKeyPress(bool capsStatus)
{
	int keyCode = 0;
	QKeyEvent *key = NULL;
	...
	if (NO_SPECIAL_KEY(text) == false
			&& (IS_BACK(text) == true || IS_BACK_EMB(text) == true
					|| IS_TAB(text) == true || IS_RETURN(text) == true
					|| IS_CTRL_LEFT(text) == true || IS_ALT(text) == true
					|| IS_CANC(text) == true || IS_CUT_LEFT(text) == true
					|| IS_PASSWORD(text) || IS_PASSWORD_EMB(text)
					|| IS_PASTE(text) || IS_COPY(text)))
		...
		if (capsStatus == false) { // se deve prendere minuscolo, controlla se il carattere è alfabetico
			...
			key = new QKeyEvent(QEvent::KeyPress, keyCode, Qt::NoModifier,
					text);
		} else
			key = new QKeyEvent(QEvent::KeyPress, keyCode, Qt::ShiftModifier,
					text);
	}
    ((DuKeyBoard *) this->m_parent)->receiptChildKey(key, NULL);
	QCoreApplication::processEvents();
}

void DuKeyPushButton::mousePressEvent(QMouseEvent * /*event */)
{
	static bool m_capsActive = true;
    DuKeyBoard *tmpKeyBoard = (DuKeyBoard *) this->m_parent;
	...
	if (IS_CAPS(text) == true) {
		...
		this->setStyleSheet(
				defaultStyleButton + changedColorButton + this->style_embedded);
		this->repaint();
		QCoreApplication::processEvents();
	} else if (tmpKeyBoard->isEnabledSwitchingEcho() == true
			&& (IS_PASSWORD(text) == true || IS_PASSWORD_EMB(text) == true))
	...
	if (tmpKeyBoard->isEmbeddedKeyboard() == true
			&& tmpKeyBoard->isZoomFacilityEnable() && NO_SPECIAL_KEY(text)
		...
		this->setGeometry(this->x() - 10,
				this->y() + adjustZoomedKey - KEY_HEIGHT_EMBEDDED,
				KEY_WIDTH_EMBEDDED + WIDTH_ZOOM_KEY,
				KEY_HEIGHT_EMBEDDED + HEIGHT_ZOOM_KEY);
		this->setStyleSheet(changedColorButton + defaultStyleButton);
		QCoreApplication::processEvents();
	}
}

void DuKeyPushButton::mouseReleaseEvent(QMouseEvent * /* event */)
{
    DuKeyBoard *tmpKeyBoard = (DuKeyBoard *) this->m_parent;
	bool pressedEcho = IS_PASSWORD(this->text()) == true
			|| IS_PASSWORD_EMB(this->text()) == true;
	...
	if (IS_CAPS(this->text()) == false && pressedEcho == false) {
		if (tmpKeyBoard->isEmbeddedKeyboard() == true
			...
			if (this->m_oldYKey != 0)
				this->setGeometry(this->x() + 10, this->m_oldYKey,
						KEY_WIDTH_EMBEDDED, KEY_HEIGHT_EMBEDDED);
			else
				this->setGeometry(this->x() + 10,
						this->y() + HEIGHT_ZOOM_KEY + KEY_HEIGHT_EMBEDDED,
						KEY_WIDTH_EMBEDDED, KEY_HEIGHT_EMBEDDED);
			this->m_oldYKey = 0;
			QCoreApplication::processEvents();
		}
